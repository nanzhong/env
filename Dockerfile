...
RUN curl -sLO https://storage.googleapis.com/kubernetes-release/release/v1.15.4/bin/linux/amd64/kubectl && chmod +x kubectl && mv kubectl /usr/local/bin/kubectl
RUN curl -sL https://get.helm.sh/helm-v3.0.0-beta.4-linux-amd64.tar.gz | tar -zvx --strip-components=1 -C /usr/local/bin linux-amd64/helm
