...
emacs:
	docker build $(DOCKER_FLAGS) -t nanzhong/emacs:$(TAG) -f emacs.Dockerfile --build-arg gitref=$(GITREF) .
ifdef PUSH
...
tmux:
	docker build $(DOCKER_FLAGS) -t nanzhong/tmux:$(TAG) -f tmux.Dockerfile --build-arg gitref=$(GITREF) .
ifdef PUSH
